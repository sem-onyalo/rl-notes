...
def get_runtime_args():
    ...
    MONTE_CARLO_V2_PARSER_DESCRIPTION = "Monte-Carlo Algorithm v2"
    ...
    for p in algorithm_parsers:
        ...
        if p.description == MONTE_CARLO_V2_PARSER_DESCRIPTION:
            ...
            drift_car_mdp_parser = mdp_parser.add_parser(DRIFT_CAR_MDP, help="An MDP for teaching an agent to drive towards a target.")
            ...
            drift_car_mdp_parser = mdp_parser.add_parser(DRIFT_CAR_MDP_V2, help="An MDP for teaching an agent to drive in a circle around a target.")
            drift_car_mdp_parser.add_argument("--render", action="store_true")
            drift_car_mdp_parser.add_argument("--discrete", action="store_true")
            drift_car_mdp_parser.add_argument("--boundary", type=int, default=4)
            drift_car_mdp_parser.add_argument("--path-radius", type=int, default=2)
        else:

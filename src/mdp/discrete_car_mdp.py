...
class DiscreteCarMDP(PyGameMDP):
    ...
    def get_state(self) -> np.ndarray:
        state = np.zeros((self.agent.view_height, self.agent.view_width), dtype=np.int32)
        return state
    ...
    def draw_viewport(self) -> None:
        ...
        for point in self.track:
            ...
            if is_point_in_view:
                viewport_point_x = viewport_left + relative_position.x
                viewport_point_y = viewport_top + relative_position.y

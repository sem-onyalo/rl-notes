...
Reinforcement learning is a subset of machine learning that involves learning what to do (mapping situations to actions) so as to maximize a numerical reward signal. It is much more focused on goal-directed learning from interaction than other machine learning approaches.
...
To solve the reinforcement learning problem a set of components are used to formulate the problem to arrive at a solution.

![Code Structure](../code-structure.png)

* Markov Decision Process (MDP): a mathematical formulation of the environment as a sequential decision making process. It can be thought of as a mathematical equation that the agent needs to solve.
* Algorithm: a representation of the agent, which determines how the agent will learn to solve the MDP (e.g. Monte-Carlo, DQN, Actor-Critic).
